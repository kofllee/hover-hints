...
public final class ArchaeologyLootCalculator {
    ...
    public static List<ArchaeologyLootEntry> calculate(
            ServerLevel world,
            BrushableBlockEntity blockEntity
    ) {
        var lootTableKey =
                ((BrushableBlockEntityAccessor) blockEntity)
                        .hoverHints$getLootTable();
        ...
        return entries.stream()
                .sorted(
                        Comparator.comparingDouble(
                                ArchaeologyLootEntry::chancePercent
                        ).reversed()
                )
                .toList();
    }
    ...
    private static void readPool(
        ...
        for (LootPoolEntryContainer poolEntry : poolEntries) {
            ...
            chancePercent =
                    Math.round(chancePercent * 10.0F)
                            / 10.0F;

            out.add(
                    new ArchaeologyLootEntry(
                            item,
                            chancePercent
                    )
            );
